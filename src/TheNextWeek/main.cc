...
scene_description& bouncing_spheres(scene_description& scene) {
...
scene_description& two_spheres(scene_description& scene) {
    scene.image_width      = 400;
    scene.sample_count     = 100;
    scene.background       = color(0.70, 0.80, 1.00);
    scene.cam.aspect_ratio = 16.0 / 9.0;
...
scene_description& two_perlin_spheres(scene_description& scene) {
    scene.image_width      = 400;
    scene.sample_count     = 100;
    scene.background       = color(0.70, 0.80, 1.00);
    scene.cam.aspect_ratio = 16.0 / 9.0;
...
scene_description& earth(scene_description& scene) {
    scene.image_width      = 400;
    scene.sample_count     = 100;
    scene.background       = color(0.70, 0.80, 1.00);
    scene.cam.aspect_ratio = 16.0 / 9.0;
...
scene_description& simple_light(scene_description& scene) {
    scene.image_width      = 400;
    scene.sample_count     = 100;
...
scene_description& cornell_box(scene_description& scene) {
...
scene_description& cornell_smoke(scene_description& scene) {
    scene.image_width      = 600;
    scene.sample_count     = 200;
    scene.background       = color(0,0,0);
    scene.cam.aspect_ratio = 1.0;
...
scene_description& final_scene(scene_description& scene) {
...
scene_description& default_scene(scene_description& scene) {
    final_scene(scene);
    scene.image_width  = 400;
    scene.sample_count = 100;
...
static color ray_color(const scene_description& scene, const ray& r, int depth) {
    hit_record rec;
    ...
    return emitted + attenuation * ray_color(scene, scattered, depth-1);
}


void render(scene_description& scene) {
    ...
    for (int j = image_height-1; j >= 0; --j) {
        ...
        for (int i = 0; i < image_width; ++i) {
            ...
            for (int s = 0; s < scene.sample_count; ++s) {
                ...
                pixel_color += ray_color(scene, r, max_depth);
            }
...
int main() {
    ...
    switch (0) {
        case 1:  render(bouncing_spheres(scene));   break;
        case 2:  render(two_spheres(scene));        break;
        case 3:  render(two_perlin_spheres(scene)); break;
        case 4:  render(earth(scene));              break;
        case 5:  render(simple_light(scene));       break;
        case 6:  render(cornell_box(scene));        break;
        case 7:  render(cornell_smoke(scene));      break;
        case 8:  render(final_scene(scene));        break;
        default: render(default_scene(scene));      break;
    }
}
